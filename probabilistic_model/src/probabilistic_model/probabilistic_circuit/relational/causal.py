...
StratificationColumns = Sequence[str]
"""
The dataframe column(s) whose joint value a stratified fit partitions its training rows
by; a single column is still a one-element sequence (``["my_column"]``), so a bare
string is never iterated character by character.
"""
...
class Stratification:
    ...
    class_columns: Optional[StratificationColumns] = None
    """
    The class-level column(s) to partition the class circuit by. ``None`` leaves the
    class circuit to the plain fit, for a cause that lives on an exchangeable part
    alone.
    """

    part_columns: Optional[Dict[str, StratificationColumns]] = None
    """
    Per exchangeable-part field name, the column(s) of that part's own template
    dataframe to partition its template fit by. Parts absent from the mapping are
    fitted plainly.
    """
