...
:class:`~coraplex.plans.executables.GiskardExecutable` builds its own controller with,
so a trajectory planned here converges the way a live Giskard motion would.
"""
...
def close_gripper_around(
    ...
    :param half_width: Half the width, in metres, the pads are to meet the object
        across, for an object grasped across a known pair of faces. Defaults to half
        the object's own bounding-box width along the closing axis, which over-reads
        the width of a box standing a little turned relative to the gripper, since the
        box's corners then reach further out than its faces do.
    """
    world = robot._world
